...
class AtomicKTCloudLBExporter:
    ...
    def __init__(self):
        ...
        # 스레드 안전성을 위한 재귀 락 (Reentrant Lock)
    ...
    def collect_data_to_temp(self):
        ...
        if not lb_list:
            logger.warning("⚠️ LB 목록이 비어있습니다.")
            return temp_data
    ...
    def atomic_update_metrics(self, temp_data):
        ...
        if not self.update_lock.acquire(blocking=False):
            ...
            logger.warning("⚠️ 메트릭 업데이트 중 충돌 감지 - 대기")
        ...
        while True:
            try:
                ...
                if sleep_time > 0:
                    ...
                    logger.warning(f"⚠️ 수집 시간이 수집 주기를 초과: {elapsed:.1f}초")
